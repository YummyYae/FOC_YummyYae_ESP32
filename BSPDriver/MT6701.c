...
esp_err_t MT6701_Init(void)
{
    ...
    const spi_device_interface_config_t dev_config = {
        // The MT6701 SSI interface is timing-sensitive, so use a conservative setup first.
...
esp_err_t MT6701_ReadAngle(uint16_t *angle_raw, float *angle_rad, uint8_t *field_status)
{
    ...
    // MT6701 SPI/SSI frame:
    // byte0[7:0] = angle[13:6]
    // byte1[7:2] = angle[5:0]
    // byte1[1:0] + byte2[7:6] carry magnetic field status bits.
