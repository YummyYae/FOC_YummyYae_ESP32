...
{
    // 把弧度制角度压到 0~2pi，再映射到 0~65535 的环形定点角度。
    while (angle >= TWO_PI) {
        angle -= TWO_PI;
    }
    while (angle < 0.0f) {
...
{
    // 这里按“电压矢量整体”做归一化和限幅，而不是分别处理 uq / ud。
    // 这样能尽量保持原始浮点实现里的电压矢量方向和幅值，避免因为量化失真
    // 导致开环电流偏大、发热增加。
...
void foc_build_sin_lut(void)
{
    ...
    for (uint32_t i = 0; i < FOC_SIN_LUT_SIZE; ++i) {
        // 这张表只在初始化阶段生成一次，快环里直接查表取 sin/cos。
...
void FOC_Calibrate_Init(void)
{
    ...
    // 上电时先进入未校准状态，是否已校准由 flash 读取或启动校准来决定。
    foc_calibrated = 0;
    foc_started = 0;
    foc_enabled = 1;

    // 初始化阶段先不输出电压，防止旧参数直接参与运行。
...
{
    foc_calibrated = 0;
    ESP_LOGW(TAG, "FOC_Calibrate_Run skipped: angle/current sensor code not connected yet");
}

void setTargetVotage(float target_q, float target_d)
{
    // 同时维护 float 和 q15 两份目标值：
    // float 方便调试观察，q15 供 ISR 快环直接使用。
